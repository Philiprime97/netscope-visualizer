...
import { Monitor, Server, Shield, Layers, Hexagon } from 'lucide-react';
import { iconNameMap } from '@/components/panels/DeviceAppearance';
...
const RouterIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
...
const defaultIconMap: Record<DeviceType, { icon: React.ElementType; className: string }> = {
  switch: { icon: Layers, className: 'text-noc-network' },
  router: { icon: RouterIcon, className: 'text-noc-network' },
  ...
  docker: { icon: DockerIcon, className: 'text-noc-container' },
...
export const DeviceIcon: React.FC<DeviceIconProps> = ({ type, size = 24, customIcon, customColor }) => {
  const defaults = defaultIconMap[type];
  const Icon = customIcon && iconNameMap[customIcon] ? iconNameMap[customIcon] : defaults.icon;
  const colorStyle = customColor ? { color: customColor } : undefined;

  return <Icon className={customColor ? '' : defaults.className} style={{ width: size, height: size, ...colorStyle }} />;
};
